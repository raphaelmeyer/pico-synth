 ...
 *   STATIC FUNCTIONS
 **********************/
static int predicate(Display * /* disp */, XEvent * /* evt */,
                     XPointer /* arg */) {
  return 1;
}

static void x11_event_handler(lv_timer_t *) {
  XEvent myevent;
  KeySym mykey;
  int n;
  ...
  while (XCheckIfEvent(display, &myevent, predicate, NULL)) {
    ...
    case Expose:
      if (myevent.xexpose.count == 0) {
        XPutImage(display, window, gc, ximage, 0, 0, 0, 0, LV_HOR_RES,
                  LV_VER_RES);
      }
      break;
    case MotionNotify:
    ...
    case ButtonRelease:
      ...
      break;
    case KeyPress:
      n = XLookupString(&myevent.xkey, &kb_buffer[0], sizeof(kb_buffer), &mykey,
                        NULL);
      kb_buffer[n] = '\0';
      break;
    case KeyRelease:
...
static void lv_x11_hide_cursor() {
  ...
  Cursor inv_cursor = XCreatePixmapCursor(display, empty_bitmap, empty_bitmap,
                                          &black, &black, 0, 0);
...
void lv_x11_flush(lv_disp_drv_t *disp_drv, const lv_area_t *area,
  ...
  if (lv_disp_flush_is_last(disp_drv)) {
    ...
    XPutImage(display, window, gc, ximage, upd_area.x1, upd_area.y1,
              upd_area.x1, upd_area.y1, upd_w, upd_h);
...
void lv_x11_get_pointer(lv_indev_drv_t *indev_drv, lv_indev_data_t *data) {
  ...
  data->state =
      left_mouse_btn ? LV_INDEV_STATE_PRESSED : LV_INDEV_STATE_RELEASED;
}
...
void lv_x11_get_mousewheel(lv_indev_drv_t *indev_drv, lv_indev_data_t *data) {
  ...
  data->state =
      wheel_mouse_btn ? LV_INDEV_STATE_PRESSED : LV_INDEV_STATE_RELEASED;
...
void lv_x11_init(char const *title, lv_coord_t width, lv_coord_t height) {
  ...
  ximage = XCreateImage(
      display, visual, dplanes, ZPixmap, 0,
      static_cast<char *>(malloc(width * height * sizeof(uint32_t))), width,
      height, 32, 0);
